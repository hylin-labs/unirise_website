...
export default function CatalogPage() {
  ...
  return <main className="detail-page"><header className="detail-nav"><a href="/">← 回到首頁</a><img src="/reference/logo.png" alt="UniRise 合軒科技有限公司" /><a href="/inquiry">詢價系統</a></header><section className="detail-hero"><div><p>PRODUCT CATALOG</p><h1>{title}</h1><span>代理品牌／產品資料</span></div></section><section className="detail-shell"><nav className="crumb"><a href="/">首頁</a><span>/</span><a href="/catalog">代理品牌</a><span>/</span><strong>{group}</strong>{item && <><span>/</span><strong>{item}</strong></>}</nav><div className="detail-grid"><img src="/reference/feature-xray-highres.png" alt="" decoding="async" /><article><div className="section-kicker">PRODUCT INFORMATION</div><h2>{title}</h2><p>{groupNotes[group] || `此頁整理原網站「${group}」分類中所列的產品名稱與服務資訊。`}</p><p className="note">完整規格、選配與適用產線需依實際需求確認。</p><a className="detail-cta" href={`/inquiry?product=${encodeURIComponent(title)}`}>詢問此產品 →</a></article></div></section></main>;
}
