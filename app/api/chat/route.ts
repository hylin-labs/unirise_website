...
export async function POST(request: Request) {
  ...
  const sources = retrieveSiteKnowledge(message);
  if (sources.length === 0) return Response.json({ answer: '目前網站沒有提供這項資訊的細節。建議您使用「詢價系統」或聯絡合軒科技（06-3319283／info-unirise@unirise.tw）。', sources: [] }, { headers: { 'Cache-Control': 'no-store' } });
  const websiteContext = sources.map((source) => `【${source.title}】\n${source.content}`).join('\n\n');
  ...
  try {
    const upstream = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        temperature: 0,
        max_tokens: 260,
        messages: [{ role: 'system', content: `${systemPrompt}\n\n網站檢索結果：\n${websiteContext}` }, ...validHistory(body.history), { role: 'user', content: message }],
      }),
    });
    ...
    return Response.json({ answer: cleanedAnswer, sources: sources.map(({ title, href }) => ({ title, href })) }, { headers: { 'Cache-Control': 'no-store' } });
  } catch {
