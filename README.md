...
4. For a clean local D1 database, apply every migration in order. The generated configuration names the local binding `DB` and local database `site-creator-d1`:
   ...
   7. `npx wrangler d1 execute site-creator-d1 --local --persist-to .wrangler/state --config dist/server/wrangler.json --file drizzle/0006_add_structured_knowledge_foundation.sql`
   8. `npx wrangler d1 execute site-creator-d1 --local --persist-to .wrangler/state --config dist/server/wrangler.json --file drizzle/0007_add_line_contact_settings.sql`
   9. `npx wrangler d1 execute site-creator-d1 --local --persist-to .wrangler/state --config dist/server/wrangler.json --file drizzle/0008_add_line_contact_qr_images.sql`
   10. `npx wrangler d1 execute site-creator-d1 --local --persist-to .wrangler/state --config dist/server/wrangler.json --file drizzle/0009_allow_qr_only_line_contacts.sql`
...
3. Use the existing Sites deploy pipeline to preview and apply that packaged migration bundle. Do not run arbitrary remote `wrangler d1 execute` commands against a production database.
   ...
   - **Fresh production database:** confirm the Sites migration preview contains all ten files in ascending order, then let the Sites deployment apply them once. The first Worker request performs the safe runtime content initialization.
