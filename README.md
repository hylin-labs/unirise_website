...
3. Use the existing Sites deploy pipeline to preview and apply that packaged migration bundle. Do not run arbitrary remote `wrangler d1 execute` commands against a production database.
   ...
   - **Fresh production database:** confirm the Sites migration preview contains all eight files in ascending order, then let the Sites deployment apply them once. The first Worker request performs the safe runtime content initialization.
